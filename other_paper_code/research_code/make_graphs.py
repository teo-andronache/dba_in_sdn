...
if combined_data:
    ...
    for i, (key, grp) in enumerate(all_data.groupby('source_file')):
        ...
        plt.plot(grp['time'], grp['demanded_mbps'], label=f"{host} Demanded BW", color=color, linestyle='--', alpha=0.6)
        plt.plot(grp['time'], grp['bandwidth_mbps'], label=f"{host} Actual BW", color=color, linestyle='-')
    ...
    plot_path = os.path.join(output_dir, "bandwidth_used_vs_demanded.png")
